/**
 * Kripto detay sayfasi tab bolumleri istemci tarafi mantigi.
 *
 * Hero guncelleme, teknik analiz render, Fear & Greed gauge ve piyasa
 * verileri render islemlerini yapar. Chart kismini stock-chart.js halleder.
 * 2dk detail polling + 1dk technical polling + 2dk fear&greed polling.
 *
 * Bagimliliklari:
 * - window.COIN_DATA (SSR)
 * - window.INITIAL_TECHNICAL (SSR)
 * - window.CHART_CONFIG.coinId (SSR)
 * - ApexCharts (radialBar gauge icin)
 */
(function () {
    'use strict';
    ...
    var FEAR_GREED_REFRESH = 120000; // 2dk
    var MAX_ERRORS = 3;
     ...
     */
    function init() {
        var config = window.CHART_CONFIG || {};
        coinId = config.coinId || (window.COIN_DATA ? window.COIN_DATA.id || '' : '');
        ...
        setInterval(function () {
            if (consecutiveErrors < MAX_ERRORS) fetchFearGreed();
        }, FEAR_GREED_REFRESH);
    }
